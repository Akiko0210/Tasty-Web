...
async function postToOrdersApi(
  ...
  if (!access_token) throw new Error("No access token — check OAuth credentials.");
...
export async function dryRunOrder(payload: OrderPayload): Promise<DryRunResult> {
  return postToOrdersApi("orders/dry-run", payload) as Promise<DryRunResult>;
}

export async function submitOrder(payload: OrderPayload): Promise<PlaceOrderResult> {
