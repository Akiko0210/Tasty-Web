...
export default function StrategyPanel() {
  ...
  function addOrder() {
    ...
    const orderId = `order-${Math.random().toString(36).substring(2, 9)}-${Date.now()}`;
    const shortId = Math.floor(100000000 + Math.random() * 900000000);
    const newOrder: Order = {
      id: orderId,
      symbol: "SPX",
      strategyName: strategyConfigs[selected].name,
      status: "Working",
      orderNumber: `Out #${shortId}`,
      tif: "Day",
      legs: legs.map((l) => ({
        ...l,
        daysToExpiry: l.daysToExpiry ?? 16,
      })),
      createdAt: new Date(),
      totalCost,
    };
